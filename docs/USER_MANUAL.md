...
この文書は、サイト管理者が対応platformへ本番インストーラーを使って導入とsudo/PAM設定を完了した環境を利用するユーザー向けです。導入、復旧、rotation、uninstallは管理者マニュアルを参照してください。
...
本番fragmentは`coding-agent`だけを対象とし、通常、login、askpass、shell、runasの各sudo formを専用PAM serviceへ通し、timestamp cacheを無効化します。`codex-fixture`は旧隔離E2E fixtureだけのテスト名で、本番ユーザー名ではありません。
...
- Ubuntu 24.04 LTS amd64以外のdistribution対応
- leaseのstatus、残り時間表示、ユーザーrevoke
- audit viewer、export、rotation
- stale socketの自動復旧
- GitHub push authority

本番導入では、サイト管理者が既存のroot復旧経路を維持し、検証済みartifactと本番インストーラーを使います。PAM/sudo fragmentだけを手作業でコピーして本番環境を構成しないでください。
