...
- 特権fixtureがホスト側で書き込む場所は
  `/var/tmp/codex-authority-task0020` だけです。事前検証済み成果物は固定された
  `/tmp/task0020-artifact-29720021660.AwGYdh` に一般ユーザーで取得します。fixtureの`/etc`、`/run`、
...
| Stage runbook SHA-256 | `5a0efd62c4f3393cd59b76930c2b76490fd61d81bfffea548eb9dd1c5d199b9b` |
| Canary runbook SHA-256 | `b71ddfce0afaa8c373e0bcd67ddfd35027ab4010bd5a87a19611921c976a0629` |
...
`unknown command "attestation"`になる場合はこのTASKを中止してください。本手順は
OS package、apt repository、keyring、`gh`、その他の前提ツールをインストールまたは
更新しません。必要なツールはTASK-0020の外で管理されたテストホストへ準備し、clean
stateから最初の確認をやり直します。attestation確認を省略して先へ進んではいけません。
